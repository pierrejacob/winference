#Constructs the summary statistic of Bonassi, F. V., West, M., et al. (2015), 
#Sequential Monte Carlo with adaptive weights for approximate Bayesian computation. Bayesian Analysis, 10(1):171–18.
library(winference)
...
upper.bound = 2000 #All values above upper.bound binned together
nbins = 50
...
#replace -Inf with -1000, and note that this is on log-scale
summary.full.matrix = pmax(summary.full.matrix,-1000)

summary.full.svd = svd(summary.full.matrix)

#Number of principal components, dimension of final summary statistic.
r = 11
M = rbind(diag(1,r),matrix(0,R+1-r,r))  
H = summary.full.svd$v%*%M #Matrix to project onto relevant principal comps.

#Function to project summaries of synthetic data sets.
