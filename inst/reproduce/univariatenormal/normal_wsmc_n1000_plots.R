...
load("~/Dropbox/ABCD/Results/univariatenormal/normaldata.n1000.wsmc.RData")
...
#############
g <- ggplot(wsmc.df %>% filter(step > 5), aes(x = mu, group = step)) + geom_density(aes(y = ..density..), colour = "darkgrey")
g <- g +  theme(legend.position = "none")
g <- g + geom_density(data = mcmc.df, aes(x = X.1, group = NULL, colour = NULL), colour = "black", linetype = 2)
g <- g + xlab(expression(mu))
g

g <- ggplot(wsmc.df %>% filter(step > 5), aes(x = sigma, group = step)) + geom_density(aes(y = ..density..), colour = "darkgrey")
g <- g + theme(legend.position = "none")
