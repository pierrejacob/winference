...
load("~/Dropbox/ABCD/Results/levydriven/levydriven.n10000.L0.wsmc_rhit-hilbert (copy).RData")
filename <- paste0("~/Dropbox/ABCD/Results/levydriven/levydriven.n",
                   nobservations, ".L", lagvalue, ".wsmc_rhit-hilbert_continue.RData")
results <- wsmc_rhit_continue(results, compute_d, target, savefile = filename)
wsmc.df <- wsmc_to_dataframe(results, target$parameter_names)
nsteps <- max(wsmc.df$step)
save(wsmc.df, results, nsteps, param_algo, file = filename)
...
# nsteps <- max(wsmc.df$step)
#
thetas <- results$thetas_history[[nsteps]]
distances <- results$distances_history[[nsteps]]
...
g <- ggplot(wsmc.df %>% filter(step > 20), aes(x = lambda, group = step, colour = step)) + geom_density(aes(y = ..density..))
g <- g +  theme(legend.position = "none")
