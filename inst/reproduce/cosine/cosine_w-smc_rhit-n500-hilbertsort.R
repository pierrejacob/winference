...
# filename <- paste0("~/Dropbox/ABCD/Results/cosine/ar1data.n", nobservations, ".metropolis.RData")
# load(filename)
...
#                colour = "black")
#
# g <- ggplot(wsmc.df %>% filter(step > 10), aes(x = rho, y = logsigma, colour = factor(step), group = step))
# g <- g + geom_point(alpha = 0.5)
# g <- g + theme(legend.position = "none")
...
#                colour = "black")
#
# g <- ggplot(wsmc.df %>% filter(step == 14), aes(x = rho, y = logsigma, colour = step, group = step))
# g <- g + geom_density2d()
# g <- g + theme(legend.position = "none")
# g <- g + xlab(expression(rho)) + ylab(expression(log(sigma)))
# g + geom_density2d(data=chain.bycomponent.df, aes(x = X.1, y = X.2, colour = NULL, group = NULL),
