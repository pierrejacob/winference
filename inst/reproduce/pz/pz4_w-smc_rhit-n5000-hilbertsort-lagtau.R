...
compute_d_hilbert <- function(theta){
  ...
  fake_obs <- create_lagmatrix(matrix(fake_obs, nrow = 1), lagvalue, tau)
  fake_obs <- fake_obs[,(lagvalue*tau+1):ncol(fake_obs)]
...
filename <- paste0("~/Dropbox/ABCD/Results/pz/pzdata.n",
                   nobservations, ".L", lagvalue, ".tau", tau, ".wsmc_rhit-hilbert.RData")
