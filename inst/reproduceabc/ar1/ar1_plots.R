...
maxstep_marginal = min(which(cumsum(results_marginal$ncomputed)>9.7*10^4))
...
g <- ggplot(wsmc.df %>% filter(step <= maxstep_marginal), aes(x = rho, y = logsigma, colour = step, group = step))
g <- g + geom_point(alpha = 0.5)
g <- g + scale_colour_gradient2(midpoint = 4.5) + theme(legend.position = "none")
...
print(g)
ggsave(filename = paste0(prefix, "ar1_identification_withoutlag.pdf"), plot = g, width = 7, height = 5)
ggsave(filename = paste0(prefix, "ar1_identification_withoutlag.png"), plot = g, width = 7, height = 5, dpi = 300)
...
maxstep_delay1 = min(which(cumsum(results_delay1$ncomputed)>9.7*10^4))
# evolution of distance threshold
...
wsmc.df <- wsmc_to_dataframe(results_delay1)
g <- ggplot(wsmc.df %>% filter(step <= maxstep_delay1), aes(x = rho, y = logsigma, colour = step, group = step))
g <- g + geom_point(alpha = 0.5)
g <- g + scale_colour_gradient2(midpoint = floor(6)) + theme(legend.position = "none")
g <- g + xlab(expression(rho)) + ylab(expression(log(sigma)))
g <- g + geom_vline(xintercept = true_theta[1]) + geom_hline(yintercept = true_theta[2])
print(g)
ggsave(filename = paste0(prefix, "ar1_identification_withlag1.pdf"), plot = g, width = 7, height = 5)
ggsave(filename = paste0(prefix, "ar1_identification_withlag1.png"), plot = g, width = 7, height = 5, dpi = 300)
