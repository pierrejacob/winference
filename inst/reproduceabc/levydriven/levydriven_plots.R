...
# })
fs = rep(0,results1$param_algo$nthetas)
for(i in 1:results1$param_algo$nthetas){
  fs[i] = f(results1$latest_y[[i]])
}
...
# g
#
# g <- ggplot(wsmc2.df %>% filter(step > 0), aes(x = omega2, y = lambda, colour = step, group = step)) + geom_point(alpha = 0.5)
